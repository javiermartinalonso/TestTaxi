 ...
 * Se aniade la deteccion de IE 7.
 */
public class Filtro_Idioma implements Filter {
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		idioma_defecto = filterConfig.getServletContext().getInitParameter("idioma_por_defecto");
	...
	public void doFilter(ServletRequest peticion, ServletResponse respuesta,
		...
		for (int i = 0; i < lista_idiomas.length; i++) {
			if (lista_idiomas[i].substring(0, 2).equals("es") && no_elegido) {
				idioma_elegido_peticion = "com/testtaxi/idiomas/textos_es";
				no_elegido = false;
			}
			if (lista_idiomas[i].substring(0, 2).equals("en") && no_elegido) {
				idioma_elegido_peticion = "com/testtaxi/idiomas/textos_en";
				no_elegido = false;
			}
			if (lista_idiomas[i].substring(0, 2).equals("fr") && no_elegido) {
				idioma_elegido_peticion = "com/testtaxi/idiomas/textos_fr";
				no_elegido = false;
			}
		}
		...
		no_elegido = true;
		if (log.isTraceEnabled()) {
			log.trace("-----******* TRATAMOS UNA PETICION DE IDIOMA INICIAL " + idioma_elegido_peticion + " ******------");
		}
